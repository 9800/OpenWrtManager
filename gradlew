...
while
    APP_HOME=${app_path%"${app_path##*/}"}  # leaves a trailing /; empty if no directory
    [ -h "$app_path" ]
do
    ls=$( ls -ld "$app_path" )
    link=${ls#*' -> '}
...
if "$cygwin" || "$msys" ; then
    ...
    # Convert default JVM opts to Windows paths.
    DEFAULT_JVM_OPTS=$( printf '%s\n' "$DEFAULT_JVM_OPTS" | xargs -n1 | sed 's~[^-[:alnum:]+,./:=@_]~\\&~g' | xargs -n1 printf '"%s" ' )
fi
